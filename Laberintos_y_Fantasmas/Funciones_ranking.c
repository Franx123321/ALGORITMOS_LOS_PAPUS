...
void mostrarRankingSDL(ContextoSDL *sdl, TTF_Font *fuente, tLista *ranking)
{
    ...
    const int tableWidth = 700;
    const int centerX = sdl->ancho / 2;
    const int leftX = centerX - tableWidth / 2;

    const int colRankW = 60;
    const int colIDW = 80;
    const int colNameW = 360;  // límite de ancho visual para el nombre
    const int colScoreW = 120;
    ...
    //const int colRankX = leftX;
    //const int colIDX = colRankX + colRankW + 30;      // antes +10
    //const int colNameX = colIDX + colIDW + 60;        // antes +20
    //const int colScoreX = colNameX + colNameW + 60;   // antes +20
    

    
    const int headerY = 120;
    ...
    while (ejecutando)
    {
        ...
        // --- TÍTULO ---
        SDL_Surface *suf = TTF_RenderText_Solid(fuente, titulo, COLOR_ROJO);
        SDL_Texture *tex = SDL_CreateTextureFromSurface(renderer, suf);
        SDL_Rect rTit = { centerX - suf->w / 2, 40, suf->w, suf->h };
        ...
        tex = SDL_CreateTextureFromSurface(renderer, suf);
        r.x = colNameX + (colNameW - suf->w) / 2; r.y = headerY; r.w = suf->w; r.h = suf->h;
        SDL_RenderCopy(renderer, tex, NULL, &r);
        SDL_FreeSurface(suf);
        SDL_DestroyTexture(tex);
        ...
        tex = SDL_CreateTextureFromSurface(renderer, suf);
        r.x = colScoreX + (colScoreW - suf->w) / 2; r.y = headerY; r.w = suf->w; r.h = suf->h;
        SDL_RenderCopy(renderer, tex, NULL, &r);
        SDL_FreeSurface(suf);
        SDL_DestroyTexture(tex);
        ...
        for (int i = 0; i < visible && act; i++, index++)
        {
            ...
            tex = SDL_CreateTextureFromSurface(renderer, suf);
            rc.x = colIDX + (colIDW - suf->w) / 2; rc.y = y; rc.w = suf->w; rc.h = suf->h;
            SDL_RenderCopy(renderer, tex, NULL, &rc);
            SDL_FreeSurface(suf);
            SDL_DestroyTexture(tex);

            // --- COLUMNA NOMBRE (recorte dinámico con “...”) ---
            char nombreTrunc[64];
            strncpy(nombreTrunc, u->nombre, sizeof(nombreTrunc) - 1);
            nombreTrunc[sizeof(nombreTrunc) - 1] = '\0';

            int textW;
            TTF_SizeText(fuente, nombreTrunc, &textW, NULL);
            while (textW > colNameW - 20 && strlen(nombreTrunc) > 3)
            {
                nombreTrunc[strlen(nombreTrunc) - 1] = '\0';
                TTF_SizeText(fuente, nombreTrunc, &textW, NULL);
            }
            if (strlen(nombreTrunc) < strlen(u->nombre))
                strcat(nombreTrunc, "...");

            suf = TTF_RenderText_Solid(fuente, nombreTrunc, COLOR_BLANCO);
            tex = SDL_CreateTextureFromSurface(renderer, suf);
            rc.x = colNameX + 10; rc.y = y; rc.w = suf->w; rc.h = suf->h;
            SDL_RenderCopy(renderer, tex, NULL, &rc);
            SDL_FreeSurface(suf);
            SDL_DestroyTexture(tex);
            ...
            tex = SDL_CreateTextureFromSurface(renderer, suf);
            rc.x = colScoreX + (colScoreW - suf->w) / 2; rc.y = y; rc.w = suf->w; rc.h = suf->h;
            SDL_RenderCopy(renderer, tex, NULL, &rc);
            SDL_FreeSurface(suf);
            SDL_DestroyTexture(tex);
        ...
        suf = TTF_RenderText_Solid(fuente, volver, COLOR_VERDE);
        tex = SDL_CreateTextureFromSurface(renderer, suf);
        SDL_Rect rVol = { centerX - suf->w / 2, sdl->alto - 80, suf->w, suf->h };
        SDL_RenderCopy(renderer, tex, NULL, &rVol);
        SDL_FreeSurface(suf);
        SDL_DestroyTexture(tex);
        ...
        while (SDL_PollEvent(&e))
        {
            ...
            else if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT)
            {
                ...
                if (mx >= rVol.x && mx <= rVol.x + rVol.w && my >= rVol.y && my <= rVol.y + rVol.h)
                    ejecutando = 0;
            }
        }
